...
Every figure below is derived from the regenerated live regression report at the bottom of this page, at the gated (largest) size of each workload. **If the two ever disagree, the live report is right and this table is stale.**
...
| registry      | 2000 |   1.29× |  1.59× | pure lower on both                                         |
...
**The claim, bounded to these rows and split by counter.** Scaling is linear on both stacks — 3.97–4.00× across a ×4 size step on every workload, gated ≤ 5.5. **On allocation**, pure-gen's per-item slope is lower on five of the six measurable rows, from 1.24× to 2.16×; the sixth, `schemaHosts` at 1.00×, is parity to within 0.5% and is **not** counted as a win. **On thunks**, pure-gen is lower on four of six and **higher on two** — `wideFreeform` by ~10% and `schemaHosts` by ~17%. The two are not the same kind of fact: `wideFreeform` thunks is by design and gated as a stated parity band, because freeform absorption is thunk-parity with nixpkgs; `schemaHosts` thunks is a carried regression, marked below. `startup` is excluded from all of this by construction — at `n=1` there is no per-item slope — and must not be read as a counterexample.

**These ratios do stand as *slope* statements, and here is why.** Five of the workloads run at two sizes in the live report, so the per-item slope ratio is recoverable from the ratio at each size plus the pure-side growth: with `pure(n) = a + s·n` on both stacks, `s_pure/s_ref = (g − 1) ÷ (g/r_big − 1/r_small)`. Across those five workloads and both counters that quantity differs from the large-`n` ratio by at most **0.002** — the measured ratio barely moves with size — so quoting the ratio and quoting the slope are the same claim to three decimals. `lazyRegistry` and `startup` run at one size only and carry no such derivation.
