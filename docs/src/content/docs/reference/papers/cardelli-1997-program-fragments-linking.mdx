...
`signature.nix`: `buildSignature` computes `{ requires, bound, unsatisfied, declaredMergeStrategies }` for each wrapped module. This is a direct analog of Cardelli's linkset structure:
