...
[doi:10.1145/325694.325708](https://doi.org/10.1145/325694.325708) · [read the paper](https://dl.acm.org/doi/pdf/10.1145/325694.325708)
...
`p` should be evaluated with its own local environment", so `let p = ?y + 2 in (p + (p with
?y = 1)) with ?y = 2` is 7, not 8. Under β-reduction of a lambda the situation is different —
