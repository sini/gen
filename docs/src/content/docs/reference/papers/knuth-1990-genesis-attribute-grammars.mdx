...
**gen-algebra** implemented convergent fixpoint computation via a `search` namespace (Palmer §3's Search monad and a `converge` runner), which is the evaluation strategy for circular attribute grammars (an extension Knuth anticipated but left to successors) — the runner iterated until stability, the same quiescence condition that AG evaluators use for circular attributes. The namespace is **retired**: gen-scope is the sole evaluator (ADR-0008 §1).
