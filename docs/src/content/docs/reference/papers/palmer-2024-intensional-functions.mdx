...
description: "Our reading of Intensional functions."
source:
...
[doi:10.1145/3689714](https://doi.org/10.1145/3689714)
...
#### gen-algebra (MAJOR) -- intensional identity; Search monad retired

**Search monad (retired, ADR-0008 §1, den-hoag-b7u1v)**: gen-algebra's `search` namespace directly implemented the intensional Search monad from S3. The `search.empty` / `search.insert` / `search.lookup` / `search.on` / `search.converge` API mirrored the paper's `insert`/`lookup`/`itsBind` operations. `search.converge` was the fixed-point loop that fired continuations on unprocessed values and repeated until stable -- the exact convergence protocol described for the intensional Search monad. Continuations created with `mkIntensional` watching the same index key were deduplicated **by name only** (`keyOf = "${indexKey}:${fn.name}"`), NOT by name+closure, which did not transfer Theorem 1's soundness guarantee — that requires equal closures *and* assumes the closure is the function's actual captured environment, whereas gen's `closure` is programmer-declared inspect data. The namespace is retired: gen-scope is the sole evaluator, and `converge` was a second one, interpreting a caller-authored continuation program on its own carrier and its own round schedule.
...
**gen-algebra: intensional hash** -- Add `intensionalHash` that produces a deterministic hash from `name` + `closure`, enabling intensional functions as keys in index structures generally (the now-retired search monad's original use case being one such structure). Scope: small. Enables gen-scope parameterized attribute caching keyed by intensional function arguments.
...
**Convergence bounds from intensional structure**: the retired search monad's convergence depended on the index growing monotonically and continuations being finite. Intensional identity provides a static bound on the number of distinct continuations (one per program point). Formalizing this could still yield tighter convergence guarantees for gen-derive fixpoint loops -- proving termination within N iterations where N is bounded by the number of identified rules.
