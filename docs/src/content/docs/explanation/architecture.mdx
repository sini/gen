...
Twenty-two libraries implement that. Every one of them sits under a single hub, and every one of
...
isn't one of these twenty-two doesn't wire into the hub at all:

```nix file=<rootDir>/lib/mkGenLibs.nix#L42-L63

```

Every one of those twenty-two belongs to exactly one of four ordered strata, plus a fifth,
